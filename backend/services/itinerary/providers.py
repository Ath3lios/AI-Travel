...
def get_weather_forecast(city: str, days: int) -> dict:
    if not OPENWEATHER_KEY:
        return {
            "city": city,
            "forecast": [
                {"day": i + 1, "condition": "Nắng nhẹ", "temp_high": 32, "temp_low": 24, "humidity": 70}
    ...
    try:
        resp = http_get(
            ...
            params={"q": city, "appid": OPENWEATHER_KEY, "units": "metric", "cnt": min(days * 8, 40), "lang": "vi"},
...
def get_top_places_from_db(db: Session, city: str, category: str, limit: int) -> list[dict]:
    ...
    if cat == "hotel":
        hotels = db.query(Hotel).order_by(Hotel.rating.desc().nullslast(), Hotel.created_at.desc()).limit(limit).all()
        for h in hotels:
            results.append({"name": h.name, "address": h.address or "", "rating": h.rating, "lat": h.lat, "lng": h.lng})
        return results

    activities_query = db.query(Activity)
    if cat:
        activities_query = activities_query.filter(Activity.category == cat)
    activities = activities_query.order_by(Activity.rating.desc().nullslast(), Activity.created_at.desc()).limit(limit).all()
    for a in activities:
        results.append({"name": a.name, "address": a.address or "", "rating": a.rating, "lat": a.lat, "lng": a.lng})
    if results:
        return results

    if cat in {"attraction", "shopping", "restaurant", "cafe"}:
        destinations = db.query(Destination).order_by(Destination.rating.desc().nullslast(), Destination.created_at.desc()).limit(limit).all()
        for d in destinations:
            results.append({"name": d.name, "address": d.city or city, "rating": d.rating, "lat": d.lat, "lng": d.lng})
    return results
...
def get_top_places(city: str, category: str, limit: int = 5) -> dict:
    ...
    if db is not None:
        ...
        if places:
            return {"city": city, "category": category, "places": places, "source": "catalog_db"}
    ...
    try:
        ...
        return {"city": city, "category": category, "error": str(e), "places": [], "source": "error"}
...
def get_exchange_rate(from_currency: str, to_currency: str) -> dict:
    try:
        ...
        resp = http_get(url, timeout=DEFAULT_TIMEOUT)
        resp.raise_for_status()
        data = resp.json()
        rate = data.get(from_lower, {}).get(to_lower)
        if rate:
            return {"from": from_currency.upper(), "to": to_currency.upper(), "rate": rate, "source": "fawazahmed0"}
        return {"from": from_currency.upper(), "to": to_currency.upper(), "error": "Không tìm thấy cặp tiền tệ", "source": "fawazahmed0"}
    except Exception:
        try:
            url2 = f"https://latest.currency-api.pages.dev/v1/currencies/{from_lower}.json"
            resp2 = http_get(url2, timeout=DEFAULT_TIMEOUT)
            resp2.raise_for_status()
            data2 = resp2.json()
            rate2 = data2.get(from_lower, {}).get(to_lower)
            if rate2:
                return {"from": from_currency.upper(), "to": to_currency.upper(), "rate": rate2, "source": "fawazahmed0-fallback"}
        except Exception:
            pass
        mock_rates = {
            ("USD", "VND"): 25400,
        ...
        return {"from": from_currency.upper(), "to": to_currency.upper(), "rate": rate_mock, "source": "mock_data"}
